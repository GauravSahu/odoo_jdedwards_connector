 ...
 */
public class ShowContactV2 extends MessageValueObject implements Serializable {

    private ContactRecordV2 contactRecord[];

    /**
     * Default public constructor for instantiating: PhoneNumberRecord<br>
     */
    public ShowContactV2() {
    }

    /**
     * Overridden public constructor for instantiating: ShowContact<br>
     */
    public ShowContactV2(InternalGetContacts internalVO, GetContactV2 pubVO) {

        if (internalVO.getQueryResults() != null) {

            this.contactRecord = 
                    new ContactRecordV2[internalVO.getQueryResults().size()]; /* create a new array big enough to hold all records in the result set */

            for (int i = 0; i < internalVO.getQueryResults().size(); 
                 i++) /* loop through all records in the result set */
            {
                this.getContactRecord()[i] = 
                        new ContactRecordV2(); /* create a new object in array */
                ...
                this.getContactRecord()[i].setContactTypeCode(qryResults.getF0111_NTYP()); 
    ...
    public void setContactRecord(ContactRecordV2[] contactRecord) {
        this.contactRecord = contactRecord;
    }

    public ContactRecordV2[] getContactRecord(int i) {
        if (i >= 0 && i < contactRecord.length)
            return contactRecord;
        return null;
    }

    public ContactRecordV2[] getContactRecord() {
        return contactRecord;
    }
}
